package view
{
    ...
    public class CutView extends Sprite
    {
        ...
        public function updateAvatar(event:MouseEvent) : void
        {
            ...
            var _pic;
            var _bigBmd = this.avatarArea.bigPic.bitmapData;
			var _newBmd,
				pSize=Param.pSize,
				oldPointer=0,
				picNames={
			...
			//生成编码容器
			var jpgEncoder:JPGEncoder = new JPGEncoder(100); //压缩质量为100
			var jpgStream = new ByteArray();
			
			jpgStream.endian=Endian.LITTLE_ENDIAN;
			
			for(var i=2,il=pSize.length;i<il;i+=2){
				_pic=this.avatarArea[picNames[i]];
				_newBmd=new BitmapData(pSize[i],pSize[i+1]);
				_newBmd.draw(_bigBmd, new Matrix(_pic.scaleX, 0, 0, _pic.scaleX, 0, 0), null, null, new Rectangle(0, 0,pSize[i],pSize[i+1]),true);
				_newBmd.applyFilter(_newBmd, new Rectangle(0, 0,pSize[i],pSize[i+1]), new Point(0, 0), this.colorAdj.filter);
				
				oldPointer=jpgStream.length;
				jpgStream.writeUnsignedInt(0);
				jpgStream.writeBytes(jpgEncoder.encode(_newBmd));
				
				jpgStream.position=oldPointer;
				jpgStream.writeUnsignedInt(jpgStream.length-oldPointer-4);
				
				jpgStream.position=jpgStream.length;
			}
